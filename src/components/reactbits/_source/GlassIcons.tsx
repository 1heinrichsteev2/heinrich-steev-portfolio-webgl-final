...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  ...
  return (
    <div
      className={`grid gap-[5em] grid-cols-2 md:grid-cols-3 justify-items-center mx-auto py-[3em] overflow-visible ${
        className || ''
      }`}
    >
      {items.map((item, index) => (
        <button
          ...
          className={`relative bg-transparent outline-none border-none cursor-pointer w-[4.5em] h-[4.5em] flex flex-col items-center [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${
            item.customClass || ''
          }`}
        >
          <span
          ...
          <span
            className="absolute top-full left-1/2 -translate-x-1/2 text-center whitespace-nowrap leading-[2] text-base opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translate(-50%,20%)]"
          >
            {item.label}
          </span>
        </button>
